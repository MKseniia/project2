...
# IMPORT -----------------
mutation_data <- read_excel("1- Raw data/Raw mutations data/ovary_mutations_final_27-08.xlsx")
...
genes <- str_remove(names(mutation_data[, 2:16]), "_mut")
...
for (i in 1:length(genes)) {
  ...
  full_data_m_gene <- full_data_m %>% select(1, 1+i, 17:22)
  ...
  if(pvalue_df < 0.05) {
    ...
    SurvPlot_df <- ggsurvplot(
      ...
      legend.labs = c("No alteration", "Alteration"),   # Change legend labels
      risk.table.height = 0.25,    # Useful to change when you have multiple groups
      ggtheme = theme_bw(),     # Change ggplot2 theme
      xlab = "Disease Free Survival (Months)", ylab = "Disease Free Survival", 
              title = paste0(gene, "ovarian cancer Disease Free survival")
    ) 
  ...
  if (pvalue_o < 0.05) {
    ...
    SurvPlot_o <- ggsurvplot(
      ...
      risk.table.height = 0.25,  # Useful to change when you have multiple groups
      ggtheme = theme_bw(),     # Change ggplot2 theme
      xlab = "Overall Survival (Months)", ylab = "Overall Survival", 
              title = paste0(gene," - ovarian cancer (Overall survival)")
    ) 
